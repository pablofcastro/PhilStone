 ...
 * with some heuristics to improve the search
 * @author Pablo
 *
 */
public class HerSearch {
	 ...
	 * @param templatePath
	 */
	public HerSearch(Spec mySpec, String outputPath, String templatePath, boolean showInfo, boolean printPDF, int scope){
		this.syntProgram = "";
		this.mySpec = mySpec;
	...
	public void startSearch(){
		System.out.println("Using Search Guided by Counterexamples with Heuristics for Synthesis...");
	...
	public boolean herSearch(int insNumber, int scope){
		...
		if (insNumber == this.numberIns - 1){ // if so we are in the base case
			if (modelCheck(currentIns, new LinkedList<LinkedList<String>>())){ // if false, we add the counterexamples to the corresponding queues
				...
				//while (cexNumber < this.cexForInstance.get(this.instancesList.get(insNumber)).size()){	
					...
					try{
						...
						Command cmd = world.getAllCommands().get(0);
						A4Solution sol = TranslateAlloyToKodkod.execute_command(rep, world.getAllReachableSigs(), cmd, opt);
						while (sol.satisfiable() & !cexRefined &  !disjointCexFound.get(currentIns)){//  &  !disjointCexFound.get(currentIns)){// & !cexRefined){ //add refined & !disjointCexFound.get(currentIns)
							if (cexRefined){
			...
			while (cexNumber < this.cexForInstance.get(currentIns).size()){
				...
				if (cexNumber == -1){ // if -1 then no counterexample will take into account
					if (herSearch(insNumber+1, scope))
						return true;
				}
				else{
					if (showInfo){
					...
					//while (cexNumber < this.cexForInstance.get(currentIns).size()){//actualCexs.size() < this.cexActualRun.get(currentIns).size()){
						...
						try{
							...
							while (sol.satisfiable()){  // !disjointCexFound.get(currentIns)??
								...
								if (herSearch(insNumber+1, scope)) // model check generates new counterexamples, TBD: we need to add any found instance to actualCexs
									return true;
	...
	private boolean modelCheck(String ins, LinkedList<LinkedList<String>> cexs){
	    ...
	    if (DCTL_MC.mc_algorithm_eq(form, model)){
	    	...
	    	if (showInfo)
	    		System.out.println("Cex from model checker: "+DCTL_MC.getBoundedWitnessAsMaps(negForm, model, ins, 100));
	    	c.addRuns(DCTL_MC.getBoundedWitnessAsMaps(negForm, model, ins, 100));
	    	//System.out.println(c);
